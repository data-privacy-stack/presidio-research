...
class TestEntityMappers:
    ...
    def test_enhanced_mapper_strategy_order(self):
        ...
        entity, confidence = mapper('NAME', ['EXACT_MATCH', 'SUBSTRING_MATCH', 'PATTERN'])
    ...
    def test_presidio_mapper_entity_naming_variations(self):
        ...
        entity, confidence = mapper('credit-card', ['CREDIT_CARD', 'PERSON', 'LOCATION'])
        assert entity == 'CREDIT_CARD'
        assert confidence > 0
...
def create_mock_analyzer():
    ...
    mock = Mock()
    mock.get_supported_entities.return_value = ["PERSON", "EMAIL_ADDRESS", "US_SSN", "LOCATION"]
    return mock
...
class TestEntityMappingComparison:
    ...
    def test_normalize_entity_for_comparison_unprefixed(self):
        ...
        assert BaseEvaluator._normalize_entity_for_comparison("STREET_ADDRESS", mapping) == "LOCATION"
        assert BaseEvaluator._normalize_entity_for_comparison("GPE", mapping) == "LOCATION"
        assert BaseEvaluator._normalize_entity_for_comparison("PHONE_NUMBER", mapping) == "PHONE_NUMBER"
    ...
    def test_normalize_entity_for_comparison_prefixed(self):
        ...
        assert BaseEvaluator._normalize_entity_for_comparison("B-STREET_ADDRESS", mapping) == "B-LOCATION"
        assert BaseEvaluator._normalize_entity_for_comparison("I-GPE", mapping) == "I-LOCATION"
        assert BaseEvaluator._normalize_entity_for_comparison("L-STREET_ADDRESS", mapping) == "L-LOCATION"
        assert BaseEvaluator._normalize_entity_for_comparison("B-PHONE_NUMBER", mapping) == "B-PHONE_NUMBER"
    ...
    def test_model_entity_mapping_raises_error(self):
        ...
        with pytest.raises(ValueError, match="entity_mapping.*deprecated.*removed from BaseModel"):
            model = MockModel(entity_mapping={"STREET_ADDRESS": "LOCATION"})
    ...
    def test_multiple_entities_map_to_same_target(self):
        ...
        tags = ["GPE", "O", "GPE", "O", "STREET_ADDRESS", "STREET_ADDRESS", "STREET_ADDRESS", "O", "ZIP_CODE"]
        prediction = ["LOCATION", "O", "LOCATION", "O", "LOCATION", "LOCATION", "LOCATION", "O", "LOCATION"]
        
        sample = InputSample(full_text="Seattle, WA, 123 Main St, 98101", tokens=tokens, tags=tags)
        model = MockTokensModel(prediction=prediction)
    ...
    def test_none_mapping_means_identity(self):
        ...
        prediction = ["B-STREET_ADDRESS", "I-STREET_ADDRESS", "I-STREET_ADDRESS", "ORGANIZATION"]
