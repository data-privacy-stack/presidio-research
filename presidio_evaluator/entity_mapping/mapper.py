...
class CanonicalMapper:
    ...
    def _build_html(self) -> str:
        """Build a self-contained HTML audit table."""
        ...
        depth_info = (
            f"Eval depth: {self._eval_depth}" if self._eval_depth else "Not analyzed"
        )
        summary_parts = [
        ...
        blocks: list[str] = []
        for severity, group in groupby(issues, key=lambda i: i.severity):
        ...
        def row_sort_key(item: tuple[str, _Resolution]) -> tuple:
            ...
            tokens = self._label_annotation_counts.get(
                lbl, 0
            ) + self._label_prediction_counts.get(lbl, 0)
            return (order.get(pt, 9), -tokens)

        rows = []
        for lbl, rec in sorted(self._records.items(), key=row_sort_key):
            pt = rec.projection_type or "NONE"
            ...
            ann_str = str(self._label_annotation_counts.get(lbl, "-"))
            pred_str = str(self._label_prediction_counts.get(lbl, "-"))
            rows.append(
    ...
    def _print_text(self) -> None:
        ...
        print(
            ...
            f"({depth_info}, {len(self._records)} labels, "
            f"{n_blocking} blocking issues)\n"
        )
        ...
        print("-" * 140)
        for lbl, rec in sorted(self._records.items()):
            canonical = rec.canonical or "-"
            projected = rec.projected or "-"
            pt = rec.projection_type or "-"
            ann = str(self._label_annotation_counts.get(lbl, "-"))
            pred = str(self._label_prediction_counts.get(lbl, "-"))
            print(
