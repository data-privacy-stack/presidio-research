...
class EntityMappingHelper:
    ...
    def _extract_all_entities(self):
        ...
        self._dataset_entity_counts = get_dataset_entities(self.dataset, include_counts=True)
        ...
        logger.debug(f"Found {len(self._all_dataset_entities)} dataset entities, "
    ...
    def _regenerate_mapping(self):
        ...
        active_dataset_entities = [
            e for e in self._all_dataset_entities 
            if e not in self._excluded_dataset_entities
        ]
        active_model_entities = [
            e for e in self._all_model_entities 
        ...
        self._unmapped_entities = [
            e for e, m in self._suggested_mapping.items() 
            if m is None and e not in self._manual_mappings
        ]
    
    def review_mapping(self, show_scores: bool = False, show_excluded: bool = False, format: str = "html"):
        """
    ...
    def _review_mapping_compact(self, show_scores: bool = False, show_excluded: bool = False):
        ...
        if self._unmapped_entities:
            ...
            print("  💡 You can: 1) map to a model entity, 2) map to None (FN penalty), or 3) exclude them.")
        else:
        ...
        if self._manual_mappings:
            ...
            manual_items = [f"{k}→{v}" for k, v in sorted(self._manual_mappings.items())]
        ...
        if show_excluded:
            ...
            if self._excluded_model_entities:
                print(f"🚫 Excluded model: {', '.join(sorted(self._excluded_model_entities))}")
    
    def _review_mapping_html(self, show_scores: bool = False, show_excluded: bool = False):
        ...
        # Build HTML
        html = []
        html.append('<div style="font-family: -apple-system, BlinkMacSystemFont, Segoe UI, Helvetica, Arial, sans-serif;">')
        
        # Header
        html.append('<h3 style="margin-top: 0; color: #24292f;">🗺️ Entity Mapping Review</h3>')
        
        # Zero-shot indicator
        if self._is_zero_shot:
            html.append('<div style="background: #ddf4ff; border-left: 4px solid #0969da; padding: 12px; margin: 10px 0; border-radius: 6px;">')
            html.append('<strong>🔄 ZERO-SHOT mode:</strong> Dataset entities will be passed to the model during evaluation.')
            html.append('</div>')
        
        # Summary stats
        html.append('<div style="display: flex; gap: 20px; margin: 15px 0; flex-wrap: wrap;">')
        html.append(f'<div style="background: #f6f8fa; padding: 10px 15px; border-radius: 6px; border: 1px solid #d0d7de;">')
        html.append(f'<strong>Dataset:</strong> {len(active_dataset_entities)} active, {len(self._excluded_dataset_entities)} excluded</div>')
        html.append(f'<div style="background: #f6f8fa; padding: 10px 15px; border-radius: 6px; border: 1px solid #d0d7de;">')
        html.append(f'<strong>Model:</strong> {len(active_model_entities)} entities</div>')
        html.append(f'<div style="background: {"#d1f4e0" if unmapped_count == 0 else "#fff8c5"}; padding: 10px 15px; border-radius: 6px; border: 1px solid {"#a2ddb8" if unmapped_count == 0 else "#d4a72c"};">')
        html.append(f'<strong>Status:</strong> {mapped_count} mapped, {unmapped_count} unmapped</div>')
        html.append('</div>')
        ...
        html.append('<details style="margin: 15px 0; background: #f6f8fa; padding: 10px; border-radius: 6px; border: 1px solid #d0d7de;">')
        html.append(f'<summary style="cursor: pointer; font-weight: 600; padding: 5px;">📊 Dataset Entities ({len(active_dataset_entities)})</summary>')
        ...
        # Mapping table
        html.append('<h4 style="margin-top: 20px; color: #24292f;">📋 Entity Mapping</h4>')
        html.append('<table style="width: 100%; border-collapse: collapse; margin: 10px 0; font-size: 13px;">')
        html.append('<thead><tr style="background: #f6f8fa; border-bottom: 2px solid #d0d7de;">')
        html.append('<th style="padding: 8px; text-align: left; border: 1px solid #d0d7de;">Dataset Entity</th>')
        html.append('<th style="padding: 8px; text-align: left; border: 1px solid #d0d7de;">→ Model Entity</th>')
        html.append('<th style="padding: 8px; text-align: center; border: 1px solid #d0d7de;">Samples</th>')
        html.append('<th style="padding: 8px; text-align: center; border: 1px solid #d0d7de;">Confidence</th>')
        ...
        for dataset_entity in sorted_entities:
            ...
            is_manually_none = is_manual and self._manual_mappings[dataset_entity] is None
            ...
            if is_manually_none:
                conf_badge = f'<span style="background: #57606a; color: white; padding: 2px 6px; border-radius: 3px; font-size: 11px; font-family: monospace;">None ✎</span>'
                ...
                conf_color = "#2da44e" if confidence >= 0.7 else ("#d4a72c" if confidence > 0.0 else "#cf222e")
                conf_badge = f'<span style="background: {conf_color}; color: white; padding: 2px 6px; border-radius: 3px; font-size: 11px; font-family: monospace;">{confidence:.2f}</span>'
                if is_manual and confidence == 1.0:
                    conf_badge = f'<span style="background: #0969da; color: white; padding: 2px 6px; border-radius: 3px; font-size: 11px; font-family: monospace;">1.00 ✎</span>'
            html.append(f'<td style="padding: 8px; border: 1px solid {border_color}; text-align: center;">{conf_badge}</td>')
            html.append('</tr>')
        
        html.append('</tbody></table>')
        
        # Add legend for confidence scores
        html.append('<div style="margin: 10px 0; padding: 10px; background: #f6f8fa; border-radius: 6px; border: 1px solid #d0d7de; font-size: 12px;">')
        html.append('<strong>Confidence legend:</strong> ')
        html.append('<span style="background: #0969da; color: white; padding: 2px 6px; margin: 0 5px; border-radius: 3px;">1.00 ✎ Manual</span>')
        html.append('<span style="background: #57606a; color: white; padding: 2px 6px; margin: 0 5px; border-radius: 3px;">None ✎ Mapped to None</span>')
        html.append('<span style="background: #2da44e; color: white; padding: 2px 6px; margin: 0 5px; border-radius: 3px;">≥0.70 High</span>')
        html.append('<span style="background: #d4a72c; color: white; padding: 2px 6px; margin: 0 5px; border-radius: 3px;">0.01-0.69 Low</span>')
        html.append('<span style="background: #cf222e; color: white; padding: 2px 6px; margin: 0 5px; border-radius: 3px;">0.00 Unmapped</span>')
        html.append('</div>')
        
        # Excluded entities (collapsible)
        if show_excluded and (self._excluded_dataset_entities or self._excluded_model_entities):
            html.append('<details style="margin: 15px 0; background: #fff1f0; padding: 10px; border-radius: 6px; border: 1px solid #ffccc7;">')
            html.append(f'<summary style="cursor: pointer; font-weight: 600; padding: 5px;">🚫 Excluded Entities</summary>')
        ...
        if self._unmapped_entities:
            html.append('<div style="background: #fff8c5; border: 2px solid #d4a72c; padding: 15px; margin: 20px 0; border-radius: 6px;">')
            html.append(f'<h4 style="margin: 0 0 10px 0; color: #9a6700;">⚠️  Action Required: {len(self._unmapped_entities)} Unmapped Entities</h4>')
            html.append('<p style="margin: 5px 0;">The following entities exist in your dataset but have no mapping:</p>')
            ...
            html.append('<li><strong>Map to None:</strong> <code>helper.set_mapping(\'ENTITY\', None)</code><br><span style="color: #57606a; font-size: 12px;">Keep in dataset but penalize model for not detecting (counted as False Negatives).</span></li>')
            html.append('<li><strong>Exclude them:</strong> <code>helper.exclude_dataset_entities([\'ENTITY\'])</code><br><span style="color: #57606a; font-size: 12px;">Remove these entities from evaluation entirely (samples filtered out).</span></li>')
            html.append('</ol></div>')
        else:
            html.append('<div style="background: #d1f4e0; border: 2px solid #2da44e; padding: 15px; margin: 20px 0; border-radius: 6px;">')
            html.append('<h4 style="margin: 0; color: #116329;">✅ All entities mapped! Ready to run experiment.</h4>')
            html.append('<p style="margin: 10px 0 0 0; color: #116329; font-size: 13px;">💡 Note: Entities mapped to <code>None</code> will be counted as False Negatives since the model doesn\'t support them.</p>')
            html.append('</div>')
    ...
    def set_mapping(self, dataset_entity: str, model_entity: Optional[str]):
        ...
        if model_entity is not None and model_entity not in self._all_model_entities:
            ...
            print(f"   Available model entities: {', '.join(sorted(self._all_model_entities))}")
            print(f"   Mapping will still be set, but the model may not recognize this entity.")
    ...
    def exclude_dataset_entities(self, entities: Union[str, List[str]]):
        ...
        if valid_entities:
            ...
            for entity in valid_entities:
                ...
                print(f"   • {entity} ({count} samples)")
            print("\n📌 Impact:")
            print("   - Samples containing these entities will be filtered out from evaluation")
            print("   - Model predictions for these entities will be ignored in all metrics")
            print()
    ...
    def get_model_to_presidio_entity_mapping(self) -> Dict[str, str]:
        """
        Generate the model-to-Presidio entity mapping for use in production.
        
        This inverts the current mapping to create a dict that can be used directly
        in NerModelConfiguration's model_to_presidio_entity_mapping parameter.
        
        The method takes the finalized dataset→model mapping and inverts it to 
        model→Presidio (dataset entities are aligned to Presidio entity names).
        
        Entities mapped to None are excluded from the result.
        
        :return: Dict mapping model entity labels to Presidio entity names
        :raises ValueError: If there are unmapped entities
        
        Example:
            >>> helper = EntityMappingHelper(dataset, model)
            >>> helper.review_mapping()
            >>> # After reviewing and modifying...
            >>> model_mapping = helper.get_model_to_presidio_entity_mapping()
            >>> # Use in TransformersNlpEngine:
            >>> ner_config = NerModelConfiguration(
            ...     model_to_presidio_entity_mapping=model_mapping
            ... )
        """
        if self._unmapped_entities:
            raise ValueError(
                f"Cannot generate mapping: {len(self._unmapped_entities)} entities are unmapped. "
                f"Resolve unmapped entities first."
            )
        
        # Invert the mapping: dataset_entity → model_entity becomes model_entity → presidio_entity
        # Since dataset entities are already aligned to Presidio names, dataset_entity = presidio_entity
        model_to_presidio = {}
        for presidio_entity, model_entity in self._suggested_mapping.items():
            if model_entity is not None and model_entity not in self._excluded_model_entities:
                model_to_presidio[model_entity] = presidio_entity
        
        return model_to_presidio
    ...
    def get_filtered_dataset(self) -> List[InputSample]:
        ...
        logger.info(f"Filtered dataset: {len(filtered)}/{len(self.dataset)} samples retained")
        return filtered
    
    def reset(self):
        """Reset all manual modifications and exclusions."""
        self._manual_mappings.clear()
        self._excluded_dataset_entities.clear()
        self._excluded_model_entities.clear()
        self._regenerate_mapping()
        print("✓ Reset all modifications")
    
    def summary(self):
        """Print a concise summary of the mapping state."""
        mapped = sum(1 for v in self._suggested_mapping.values() if v is not None)
        unmapped = len(self._unmapped_entities)
        
        print(f"\nMapping Summary:")
        print(f"  • {mapped} entities mapped")
        print(f"  • {unmapped} entities unmapped" + (" ⚠️" if unmapped > 0 else ""))
        print(f"  • {len(self._excluded_dataset_entities)} dataset entities excluded")
        print(f"  • {len(self._excluded_model_entities)} model entities excluded")
        print(f"  • {len(self._manual_mappings)} manual mappings set")
        
        if unmapped == 0:
            print("\n✓ Ready to run experiment!")
        else:
            print(f"\n⚠️  Action required: {unmapped} entities need mapping or exclusion")
...
def get_model_entities(model: Any, language: str = "en") -> List[str]:
    ...
    if hasattr(model, "analyzer_engine") and hasattr(model.analyzer_engine, "get_supported_entities"):
        try:
            entities = model.analyzer_engine.get_supported_entities(language)
            logger.info(f"Detected {len(entities)} entities from wrapped Presidio Analyzer")
            return entities
        except Exception as e:
    ...
    if hasattr(model, "config"):
        ...
        if hasattr(config, "id2label") and config.id2label is not None:
            try:
                ...
                logger.info(f"Detected {len(entities)} entities from Transformers config.id2label")
                return entities
            ...
            try:
                ...
                for label in labels:
                    ...
                    if entity and entity != "O":
                        entities.add(entity)
                entities = sorted(entities)
                logger.info(f"Detected {len(entities)} entities from Transformers config.label2id")
                return entities
            except (TypeError, AttributeError) as e:
    ...
    if hasattr(model, "pipe_names") and "ner" in model.pipe_names:
        try:
            ...
            if hasattr(ner_component, "labels"):
                ...
                logger.info(f"Detected {len(entities)} entities from SpaCy NER component")
                return entities
        except Exception as e:
    ...
    if hasattr(model, "ner_model_configuration") or hasattr(model, "_engines"):
        try:
            ...
            if entities:
                logger.info(f"Detected {len(entities)} entities from TransformersNlpEngine")
                return entities
        except Exception as e:
    ...
    if not entities:
        logger.warning("Could not automatically detect model entities. Please provide them manually.")
        return []
...
def _get_transformers_nlp_engine_entities(nlp_engine: Any) -> List[str]:
    ...
    if hasattr(nlp_engine, "ner_model_configuration"):
        ...
        if config and hasattr(config, "model_to_presidio_entity_mapping"):
            ...
            if mapping:
                ...
                logger.info(f"Found entities from NerModelConfiguration mapping: {sorted(entities)}")
                if entities:
    ...
    if hasattr(nlp_engine, "_engines"):
        for lang_code, engine in nlp_engine._engines.items():
            ...
            if entities:
                logger.info(f"Found entities from transformer model config: {sorted(entities)}")
                break
...
def print_mapping_summary(
    dataset_entities: Union[List[str], Dict[str, int]],
    model_entities: List[str],
    mapping: Dict[str, Optional[str]],
    scores: Optional[Dict[str, float]] = None,
):
    """
    Print a formatted summary of the entity mapping.
    
    :param dataset_entities: List or dict (with counts) of dataset entity types
    :param model_entities: List of model entity types
    :param mapping: Dict mapping dataset entities to model entities
    :param scores: Optional similarity scores for each mapping
    ...
    Example output:
        ...
        ✓ FIRST_NAME      → PERSON       (100 occurrences) [score: 0.92]
        ✓ LAST_NAME       → PERSON       (98 occurrences)  [score: 0.89]
        ✓ EMAIL           → CONTACT      (120 occurrences) [score: 0.95]
        ✗ CUSTOM_ID       → (no match)   (45 occurrences)
        
        Mapping success rate: 75.0% (6/8 entities mapped)
        ```
    """
    # Convert list to dict if needed
    if isinstance(dataset_entities, list):
        entity_counts = {e: None for e in dataset_entities}
    else:
        entity_counts = dataset_entities
    
    # Calculate stats
    total_dataset_entities = len(entity_counts)
    total_model_entities = len(model_entities)
    mapped_count = sum(1 for v in mapping.values() if v is not None)
    unmapped_count = total_dataset_entities - mapped_count
    success_rate = (mapped_count / total_dataset_entities * 100) if total_dataset_entities > 0 else 0
    ...
    print("=" * 70)
    print()
    print(f"Dataset entities: {total_dataset_entities}")
    print(f"Model entities: {total_model_entities}")
    print()
    
    # Print model entities
    print("MODEL SUPPORTED ENTITIES:")
    for entity in sorted(model_entities):
        print(f"  • {entity}")
    print()
    ...
    # Sort by: unmapped first, then by entity name
    sorted_entities = sorted(
        entity_counts.keys(),
        key=lambda e: (mapping.get(e) is not None, e)
    )
    
    for dataset_entity in sorted_entities:
        model_entity = mapping.get(dataset_entity)
        count = entity_counts[dataset_entity]
        
        # Format entity names with padding
        source_str = f"{dataset_entity:<18}"
        
        # Build the mapping line
        if model_entity:
            status = "✓"
            target_str = f"→ {model_entity:<15}"
        else:
            status = "✗"
            target_str = f"→ {'(no match)':<15}"
        
        # Add count if available
        count_str = ""
        if count is not None:
            count_str = f"({count:>4} occurrences)"
        
        # Add score if available
        score_str = ""
        if scores and dataset_entity in scores:
            score = scores[dataset_entity]
            score_str = f"[score: {score:.2f}]"
        
        print(f"  {status} {source_str} {target_str} {count_str:>18} {score_str}")
    
    print()
    print("-" * 70)
    print(f"Mapping success rate: {success_rate:.1f}% ({mapped_count}/{total_dataset_entities} entities mapped)")
    
    if unmapped_count > 0:
        print(f"\n⚠️  WARNING: {unmapped_count} entities could not be mapped automatically.")
        print("   These entities will be ignored during evaluation unless you provide")
        print("   a custom mapping.")
    
    print("=" * 70)
    print()


def save_mapping_to_file(mapping: Dict[str, str], filepath: Union[str, Path]):
    """
    Save entity mapping to a JSON file.
...
def load_mapping_from_file(filepath: Union[str, Path]) -> Dict[str, str]:
    """
    ...
    with open(filepath, 'r', encoding='utf-8') as f:
        mapping = json.load(f)
    print(f"✓ Mapping loaded from {filepath}")
    return mapping
