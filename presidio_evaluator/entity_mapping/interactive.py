...
class EntityMappingHelper:
    ...
    def get_model_to_presidio_entity_mapping(self) -> Dict[str, str]:
        ...
        for presidio_entity, model_entity in self._suggested_mapping.items():
            if (
                model_entity is not None
                and model_entity not in self._excluded_model_entities
            ):
...
def print_mapping_summary(
    ...
    success_rate = (
        (mapped_count / total_dataset_entities * 100)
        if total_dataset_entities > 0
        else 0
    )
    ...
    sorted_entities = sorted(
        entity_counts.keys(), key=lambda e: (mapping.get(e) is not None, e)
    )
    ...
    print("-" * 70)
    print(
        f"Mapping success rate: {success_rate:.1f}% ({mapped_count}/{total_dataset_entities} entities mapped)"
    )
